...
import { supabase, SupplierOrder } from '@/lib/supabase';
...
export default function SupplierOrdersPage() {
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      // Explicitly exclude total_amount_to_pay from the query
      const { data, error } = await supabase
        .from('orders')
        .select('id, created_at, updated_at, ordered_date, customer_name, address, contact, product_link, image_url, size, color, material, notes, tracking_number, is_paid, is_shipped, is_completed_on_etsy, is_delivered, issue, the_solution, internal_notes')
  ...
  const handleFieldUpdate = async (orderId: string, field: keyof SupplierOrder, value: any) => {
    try {
      const { error } = await supabase
        .from('orders')
        .update({ [field]: value })
  ...
  const handleToggle = async (orderId: string, field: keyof SupplierOrder, currentValue: boolean | undefined) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Orders Table - Same as Admin but WITHOUT Amount column */}
        ...
        {/* Summary Footer - WITHOUT financial data */}
        {filteredOrders.length > 0 && (
        ...
        <div className="mt-4 bg-blue-50 rounded-lg p-4 border border-blue-200">
          <p className="text-sm text-blue-900 text-center">
            Supplier View - Financial data is not displayed for security purposes.
          </p>
        </div>
      </div>
    </div>
